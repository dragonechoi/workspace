...
public class ListTest {
	...
	public static void main(String[] args) {
		...
		// Collection 클래스들 : 자료구조를 구현한  Java의 API 클래스들 - 대량의 데이터를 관리 [교재. p.458참고]
		
		// Collection인터페이스의 3가지 종류 : List, Set, Map
		// 1) List : 순서대로 저장, 인덱스 번호 O, 중복 O
		// 2) Set  : 순서 X , 인덱스 X, 중복 X
		// 3) Map  : 순서 X , 익덱스 번호 대신 Key식별자 사용, key는 중복X, value는 중복O
		
		// 3종류 모두 인터페이스이기에 그냥 사용하는 것이 아니라 이를 구현한 하위 클래스들을 사용
		
		// List 인터페이스를 구현한 하위클래스 3개 : ArrayList, LinkedList, Vector
		// 3개 모두 사용문법을 거의 같음. 단지 성능이 다른것뿐.
		...
		//6. 제거할때 인덱스 번호가 아니라 객체의 주소값(참조변수)으로 제거 가능
		...
		//요소값을 확인하기위서는 반복문을 사용하여 .get()을 여러번..
		...
		// 리스트를 배열로 변환
		String[] aaa= new String[list.size()]; //리스트개수와 같은 크기의 요소값이 없는 빈 배열
		...
		// 빈번하게 요소를 추가/삭제를 할때 속도가 빠름. 반대로 특정요소값을 얻어올때는 느림
		...
		// Vector 클래스 - ArrayList와 완전 같음. [ 동기화 처리가 되어 있다는 차이만 있음. Thread를 배워야 동기화를 이해할 수 있]
