...
public class FileOutputTest {
	...
	public static void main(String[] args) {
		...
		// 데이터를 영구적으로 저항할 HDD는 File형태로 데이터를 저장함
		//먼저 File을 제어하는 클래스 객체로 생성
		File file= new File("aaa.txt");// 파일이 저장된 경로(폴더)를 지정하지 않으면 자동 현재 프로젝트폴더에 파일이 위치함.
		
		/// 파일이 저장될 경로를 지정해보기..
		file= new File("kkk\\aaa.txt"); //windows에서는 역슬래시(\)가 폴더구분자. [단, 문자열""안에서는 \ 를 특수문자로 보기에 2번써야 \로 인식함]
		file= new File("kkk/aaa.txt");//MacOS or Linux 는 경로구분자로 슬래스를 사용함[단, windows는 슬래스도 됨]
		...
		//내 프로젝트폴더가 아니라. HDD의 특정 파티션 영역사용
		...
		// 그 중에서도 파일전용으로 설계된 FileOutpuStream클래스로 객체 생성
		try {
